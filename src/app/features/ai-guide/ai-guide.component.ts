    ...
    <div class="flex flex-col h-screen bg-background">
      ...
      <div #scrollContainer class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        @if (messages().length === 0) {
          <div class="max-w-xl mx-auto py-8 space-y-7">

            <div class="text-center space-y-2">
              <div class="w-16 h-16 rounded-3xl bg-stone-100 border border-black/[0.05] flex items-center justify-center mx-auto mb-4 shadow-subtle">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              ...
              >
                <div class="w-9 h-9 rounded-2xl bg-stone-100 text-stone-900 flex items-center justify-center text-lg flex-shrink-0 group-hover:scale-105 transition-transform">
                  🍽️
                </div>
        ...
        <div class="max-w-xl mx-auto space-y-4">
          @for (msg of messages(); track msg.id) {
            <div class="flex items-start gap-2.5" [ngClass]="{ 'flex-row-reverse': msg.role === 'user' }">

              @if (msg.role === 'assistant') {
                <div class="w-7 h-7 rounded-full bg-stone-100 border border-black/[0.06] flex items-center justify-center flex-shrink-0 mt-1 shadow-sm">
                  <lucide-icon [img]="BotIcon" [size]="14" class="text-gold" strokeWidth="1.5"></lucide-icon>
                </div>
              }

              <div class="space-y-3 max-w-[85%] sm:max-w-[80%]">
                ...
                @if (msg.role === 'assistant' && shouldShowRestaurantCard(msg.content)) {
                  <div class="bg-white rounded-3xl border border-black/[0.06] shadow-luxe overflow-hidden transition-all">
                    <div class="relative h-36">
                      ...
                      />
                      <div class="absolute top-3 left-3 bg-obsidian/90 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1 rounded-full flex items-center gap-1">
                        <lucide-icon [img]="MapPinIcon" [size]="10" strokeWidth="1.5"></lucide-icon>
                        120 m uzaklıkta
                      </div>
                      <div class="absolute top-3 right-3 bg-gold text-stone-950 text-[10px] font-bold px-2 py-0.5 rounded-md shadow-sm">
                        4.6 ★
                      </div>
                    </div>
                ...
                @if (msg.role === 'assistant' && shouldShowRouteCard(msg.content)) {
                  <div class="bg-white rounded-3xl border border-black/[0.06] shadow-luxe overflow-hidden transition-all">
                    ...
                    <div class="p-3.5 space-y-2.5">
                      <div class="space-y-1.5 text-xs text-stone-800">
                      ...
                      <div class="pt-2 border-t border-black/[0.04] flex justify-between items-center">
          ...
          @if (isLoading()) {
            <div class="flex items-end gap-2">
              <div class="w-7 h-7 rounded-full bg-stone-100 border border-black/[0.06] flex items-center justify-center flex-shrink-0">
                <lucide-icon [img]="BotIcon" [size]="14" class="text-gold" strokeWidth="1.5"></lucide-icon>
              </div>
              <div class="bg-white border border-black/[0.06] px-4 py-3 rounded-2xl rounded-bl-xs shadow-subtle">
      ...
      >
        <div class="max-w-xl mx-auto space-y-2">
          ...
          @if (isListening()) {
            <div class="flex items-center justify-between px-3 py-1.5 bg-stone-900 text-white rounded-xl text-xs font-medium animate-pulse">
